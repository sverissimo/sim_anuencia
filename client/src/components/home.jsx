import React from 'react';
import { Link } from 'react-router-dom';

const Home = () => {

  return (

<div>
<div className="card-panel" id="jumbotron_home">
        <div className="container">
          <h1 className="display-3"> <strong> <span style={{ color: 'white' }} >Bienvenido a SIM</span> </strong> </h1>
          <h5 style={{ color: 'white' }}>
           Módulo Experimental Anuência Prévia (versão beta).
          </h5>
          <p className="btn btn-primary btn-lg" href="" role="button">Saiba mais &raquo;</p>
        </div>
      </div>


        <hr />



<div className="container">


  <div className="row">
    <div className="col s12 m4">
      <img  src="data:image/png;base64," style={{ margin: '10px' }} alt="" width="105" height="120"  />
        <h4>Cadastrar Processo</h4>
        <p style={{textAlign: 'justify'}}>Donec sed odio dui. Etiam porta sem malesuada magna mollis euismod. Nullam id dolor id nibh ultricies vehicula ut id elit. Morbi leo risus, porta ac consectetur ac, vestibulum at eros. Praesent commodo cursus magna.</p>
        <p className="btn" href="" role="button">
        <Link to="/cadastro_emp" style={{ color: 'white' }}>Cadastrar Processo</Link> 
        &raquo;</p>
    </div>
    <div className="col s12 m4">
        <img  src="https://cdn0.iconfinder.com/data/icons/science-9/512/list-512.png" alt="" width="140" height="140" />
          <h4>Solicitar Anuência</h4>
          <p style={{textAlign: 'justify'}} >Duis mollis, est non commodo luctus, nisi erat porttitor ligula, eget lacinia odio sem nec elit. Cras mattis consectetur purus sit amet fermentum. Fusce dapibus, tellus ac cursus commodo, tortor mauris condimentum nibh.</p>
          <p className="btn" href="" role="button">
          <Link to="/cadastro_rt" style={{ color: 'white' }}>Solicitar Anuência &raquo;</Link></p>
    </div>
    <div className="col s12 m4">
          <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZ3uBJUOmCdcrPDvHxrcMzLCYCYDzQqHCOB_VRys6jKUQXHN4P" alt="" style={{ margin: '10px' }} width="120" height="120" />
            <h4>Buscar Processo</h4>
            <p style={{textAlign: 'justify'}}>Aqui você pode acompanhar o andamento a situação atual do processo de loteamento ou desmembramento, desde seu cadastro até a emissão da anuência prévia.</p>
            <p className="btn" href="" role="button">
            <Link to="/showEmpreend" style={{ color: 'white' }}>Buscar Processo &raquo;</Link></p>
    </div>
  </div>
  </div>
</div>

   

            )
        
        };
        
export default Home;